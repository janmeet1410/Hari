...
export default class InsightPage extends React.Component<IInsightPageProps, IInsightPageState> {
  ...
  public render(): React.ReactElement<IInsightPageProps> {
    ...
    return (
      ...
      {Object.entries(this.groupByCategory(this.state.aiInsights)).map(
  ([category, items]: [string, any[]], catIndex) => (
    <div key={catIndex}>
      <h2>{category}</h2>
            ...
            >
              <div
                className={`case-header ${
                  ind % 3 === 0 ? "blue" : ind % 3 === 1 ? "green" : "purple"
                }`}
              >
                <div className="category" style={{ backgroundColor: bgColor, width:'max-content' }} >{el.Department}</div>
                <h3>{el.Title}</h3>
              </div>
